...
public class ApiISBNGeneral {
	 ...
	 */
	public Comic getBookInfo(String isbn, TextArea prontInfo) {
		...
		try {
			...
			StringBuilder escritores = new StringBuilder();

			for (String a : artistasJson) {
				artistas.append(a.replace("-", "") + " - ");
			}

			for (String e : escritoresJson) {
				escritores.append(e.replace("-", "") + " - ");
			}

			String nombre = extraerTitulo(details);
			String keyIssue = extraerDescripcion(details);
			String dibujantes = artistas.toString();
			String guionistas = escritores.toString();
			String numero = "0";
			String precio = "0";
			String variant = "";
	...
	private String extraerReferencia(JSONObject bookInfo) throws JSONException {
		...
		if (bookInfo.has("info_url")) {
			return bookInfo.getString("info_url");
		} else {
